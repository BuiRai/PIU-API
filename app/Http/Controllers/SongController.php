...
class SongController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        $query = Song::with('artist', 'gameVersion', 'type');

        if ($request->has('page')) {
            if ($request->has('search')) {
                $songs = $query->search($request->input('search'))->paginate(10)->items();
            } else {
                $songs = Cache::remember('CacheSongs_page_' . $page, 20/60, function() use($query){
                    return $query->paginate(10)->items();
                });
            }
        } else {
            $songs = Cache::remember('CacheSongs_full', 20/60, function() use($query){
                return $query->get();
            });
        }
    ...
    */
    ...
    {
        $song = Song::with('artist', 'gameVersion', 'levels.style', 'type', 'channels')->find($id);
